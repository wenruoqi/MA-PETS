...
class Controller:
    ...
    def act(self, obs, t, agent_id, get_pred_cost=False):
        """Performs an action.
        """
        raise NotImplementedError("Must be implemented in subclass.")
...
class MPC(Controller):
    ...
    def __init__(self, params):
        ...
        self.dO, self.dU = 8, 1 
        self.ac_ub, self.ac_lb=np.array([13.89], dtype=np.float32),np.array([0], dtype=np.float32) 
        self.update_fns = []
        self.per = 1
        ...
        sol = np.tile((self.ac_lb + self.ac_ub) / 2, [self.plan_hor])
        self.prev_sol = {}
        var = np.tile(np.square(self.ac_ub - self.ac_lb) / 16, [self.plan_hor]) 
        self.init_var = {}
        for agent_id in MPC.AGENT_IDS:
            self.prev_sol.update({agent_id : sol}) 
            self.init_var.update({agent_id : var})
        self.train_in = np.array([]).reshape(0, self.dU + self.obs_preproc(np.zeros([1, self.dO])).shape[-1])
        self.train_targs = np.array([]).reshape(0, np.zeros([1, self.dO]).shape[-1])
    ...
    def train(self, new_train_in, new_train_targs):
    ...
    def act(self, obs, t, agent_id, get_pred_cost=False):
        ...
        Arguments:
            ...
            agent_id: added, to seperate different ego agents

        Returns: An action (and possibly the predicted cost)
        """
        if not self.has_been_trained:
            out = np.random.uniform(self.ac_lb, self.ac_ub, self.ac_lb.shape)
            return out
        ...
        soln = self.optimizer.obtain_solution(self.prev_sol[agent_id], self.init_var[agent_id])
        
        self.prev_sol[agent_id] = np.concatenate([np.copy(soln)[self.per * self.dU:], np.array((self.ac_lb+self.ac_ub)/2)])
        self.ac_buf = soln[:self.per * self.dU].reshape(-1, self.dU)
 
        return self.act(obs, t, agent_id)
    ...
    def _compile_cost(self, ac_seqs):
        ...
        for t in range(self.plan_hor):
            ...
            cost = (next_obs[:,0] + next_obs[:,4] + next_obs[:,5]) - next_obs[:,3] *30
    ...
    def _predict_next_obs(self, obs, acs):
        ...
        inputs = torch.cat((proc_obs, acs), dim=-1).to(TORCH_DEVICE) 
